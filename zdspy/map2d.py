...
class MAP2D_PAL_CE:
    def __init__(self, data):
        ...
        # Little Endian
        self.bitstring = (
            bin(int(str(self.data.hex()[2:]), base=16))[2:].zfill(8)
            + ""
            + bin(int(str(self.data.hex()[:2]), base=16))[2:].zfill(8)
        )
    ...
    def get_rgb(self):
        return (
            int((int(self.red, base=2) / 31) * 255),
            int((int(self.green, base=2) / 31) * 255),
            int((int(self.blue, base=2) / 31) * 255),
        )
...
class MAP2D:
    def __init__(self, data):
        ...
        if ndspy:
            ...
            debug_print(
                "Please install ndspy to use this feature! (https://github.com/RoadrunnerWMC/ndspy)"
            )
            return None
    ...
    for directory in dirs:
        for _r, _d, f in os.walk(directory):
            for file in f:
                ...
                MAP2D(d.ReadFile(directory + "/" + file)).save_bitmap(
                    outdir, name + "_" + file[:-4], path_tiles="tiles/", path_palette="palettes/"
                )
