...
_NUMBER_OF_BYTES_TO_STRUCT_CHARACTER = {1: "B", 2: "H", 4: "I"}


def _get_pack_fmt(number_of_bytes: int, is_little_endian: bool, is_signed: bool) -> str:
    endianess_symbol = "<" if is_little_endian else ">"
    pack_fmt = endianess_symbol + (
        _NUMBER_OF_BYTES_TO_STRUCT_CHARACTER[number_of_bytes].lower()
        if is_signed
        else _NUMBER_OF_BYTES_TO_STRUCT_CHARACTER[number_of_bytes]
    )
    return pack_fmt
