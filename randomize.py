...
from zdspy.helpers import ZDS_PH_AREA, ZDS_PH_MAP
...
def nologic(warp_list):
    ...
    for i in range(len(warp_list)):
        ...
        n_warp_ce = zds.ZMB_WARP_CE(
            warp_ce.UID,
            warp_ce.fade_type,
            p_warp_ce.map_id,
            p_warp_ce.destination_warp_id,
            p_warp_ce.destination,
            warp_ce.run_direction,
        )
...
def nologicdual(warp_list):  # Similar to nl
    ...
    for i in range(int(round(len(warp_list) / 2))):
        ...
        n_warpl[p_fname] = zds.ZMB_WARP_CE(
            p_warp_ce.UID,
            p_warp_ce.fade_type,
            warp_ce.map_id,
            warp_ce.destination_warp_id,
            warp_ce.destination,
            p_warp_ce.run_direction,
        )
...
def nologiclinked(warp_list):
    ...
    for i in range(int(round(len(warp_list) / 2))):
        ...
        # uid, ft, mid, dwid, dest, rundir
        new_warp_list[fname] = zds.ZMB_WARP_CE(
            warp_ce.UID,
            warp_ce.fade_type,
            int(p_info.map_id),
            p_warp_ce.destination_warp_id,
        ...
        new_warp_list[p_fname] = zds.ZMB_WARP_CE(
            p_warp_ce.UID,
            p_warp_ce.fade_type,
            int(info.map_id),
            warp_ce.destination_warp_id,
